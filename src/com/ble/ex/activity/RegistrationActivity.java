...
import com.ble.ex.R;
import com.ble.ex.client.ApplicationEnvironment;
import com.ble.ex.client.BLEUtil;
import com.ble.ex.client.Constants;
...
public class RegistrationActivity extends BaseActivity implements OnClickListener {
	...
	private boolean checkValue() {
		...
		for (Iterator<String> it = devices.iterator(); it.hasNext();) {
			String str = it.next();
			Log.e("==", str.split("&")[0]);
			Log.e("==", this.deviceNumText.getText().toString());
			if (str.split("&")[0].equalsIgnoreCase(BLEUtil.getSimpleDeviceName(this.deviceNumText.getText().toString()))) {
	...
	private BluetoothAdapter.LeScanCallback mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
		...
		public void onLeScan(final BluetoothDevice device, int rssi, byte[] scanRecord) {
			...
			if (deviceName.equals(matchName)) {
				...
				newset.addAll(deviceSet);
				newset.add(deviceName + "&" + device.getAddress());

				Editor editor = ApplicationEnvironment.getInstance().getPreferences().edit();
				editor.putStringSet(Constants.MY_DEVICELIST, newset);
				editor.commit();
